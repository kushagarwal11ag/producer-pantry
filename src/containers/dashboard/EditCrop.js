...
const AddProduct = ({ cropId }) => {
	...
	useEffect(() => {
		const fetchCrop = async () => {
			...
			setCredentials({
				...
				quantity: cropDetails.quantity || 0,
				available: String(cropDetails.available) || "true",
			});

			setImageURL(cropDetails.image);
		};
		fetchCrop();
	}, []);
	...
	const handleSubmit = async (event) => {
		...
		try {
			...
			formData.append("available", credentials.available === "true");
			if (postFile) {
